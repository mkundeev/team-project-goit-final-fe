...
export default function Chart({ result }) {
  ...
  const dataChart = [
    {
      title: 'Incorrect',
      value: (rejectAnswer / 12) * 100,
      color: '#D7D7D7 ',
    },
    {
      title: 'Correct',
      value: (correctAnswer / 12) * 100,
      color: '#ff6b09',
    },
  ];
    ...
    <div className={s.ChartTest}>
      {result && (
        <Container>
          ...
          <ul className={s.list}>
            <li className={s.item}>Correct answers - {correctAnswer}</li>
            <li className={s.item}>
              Total questions - {correctAnswer + rejectAnswer}
            </li>
          </ul>
          <SimonCat data={correctAnswer} />
